...
import cn.minishop.shop.dao.IOrdersDAO;
...
public class OrdersDAOImpl extends AbstractDAOImpl implements IOrdersDAO {
    ...
    public Orders findById(Integer id) throws Exception {
        Orders o = null;
        String sql = "SELECT oid,mid,name,phone,address,creDate,totalPay FROM orders WHERE oid=?";
    ...
    public List<Orders> findAllSplit(Integer currentPage, Integer lineSize, String column, String keyWord) throws Exception {
        ...
        String sql = "SELECT oid,mid,name,phone,address,creDate,totalPay FROM orders WHERE "+ column +" LIKE ? LIMIT ?,?";
        super.pstmt = super.conn.prepareStatement(sql);
    ...
    public boolean doCreateOrders(Orders vo) throws SQLException {
        String sql = "INSERT INTO orders(mid,name,phone,address,creDate,totalPay) VALUES (?,?,?,?,?,?)";
        super.pstmt = super.conn.prepareStatement(sql);
    ...
    public List<Orders> findAllByMember(String mid, Integer currentPage, Integer lineSize) throws Exception {
        List<Orders> all = new ArrayList<Orders>();
        String sql = "SELECT oid,mid,name,phone,address,creDate,totalPay FROM orders WHERE mid=? LIMIT ?,?";
        super.pstmt = super.conn.prepareStatement(sql);
    ...
    public Orders findByIdAndMember(String mid, Integer oid) throws Exception {
        Orders o = null;
        String sql = "SELECT oid,mid,name,phone,address,creDate,totalPay FROM orders WHERE mid=? AND oid=?";
        super.pstmt = super.conn.prepareStatement(sql);
        super.pstmt.setString(1,mid);
